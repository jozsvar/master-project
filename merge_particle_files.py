...
parser = argparse.ArgumentParser(description = '''A program that merges all files from Plates 1, 2, 3, 4, 6, 7, 8''')
...
def merge_plates(plate_dir, prefix, letters, well_numbers, picture_numbers, outdir):
    ...
    #pdb.set_trace()
    results.to_csv(outdir+'particle_merged.tsv', index=False, sep='\t')

    print 'Particle files merged'
